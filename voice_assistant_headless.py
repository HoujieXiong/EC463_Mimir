...
while True:
    ...
    if trackOn:
        try:
            frame = rgb_queue.get(timeout=0) # grabs latest RGB frame
            img = frame.getCvFrame() #Converts to numpy array to process with CV2
            fingertips = detect_fingertips(img)
            if fingertips:
                word, bbox = extract_text(img, fingertips['index']) #Forms bounding box around finger tip
                if word:
                    say_text_espeak(word) #If word is found then espeak it 
        except queue.Empty:
            pass #Do nothing
